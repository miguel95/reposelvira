...
class PersonaDao(DBcon.DBcon):
    ...
    def eliminarPersona(self, estado):
        con=self.conexion().connect()
        sql=""" DELETE FROM personas  WHERE  estado = '%i'""" % (estado)
        print sql
        with con:
            cursor=con.cursor()
            cursor.execute(sql)
    ...
    def editarPersona(self, nombre, apaterno, amaterno, cedula, fnacimiento, sexo, direccion, celular, estado):
        ...
        sql=""" UPDATE set personas where '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %i""" % (nombre, apaterno, amaterno, cedula, fnacimiento, sexo, direccion, celular, estado)
        print sql
        with con:
            cursor=con.cursor()
            cursor.execute(sql)
